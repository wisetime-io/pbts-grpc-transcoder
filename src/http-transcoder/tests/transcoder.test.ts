...
test("TestMultipleBindings - Transcoder should pick the main binding", () => {
  const substitution = randomString()
  const requestMessage = TestApi.SimpleFieldMessage.create({ field: substitution.unencoded })
  const httpRequest = transcode("TestPathVariable", requestMessage)

  expect(httpRequest.method).toEqual("GET")
  expect(httpRequest.urlEncodedPath).toEqual(`/v1/${substitution.percentEncoded}/bar`)
...
test("TestPathVariableWithStar substitution", () => {
  const substitution = randomString()
  const requestMessage = TestApi.SimpleFieldMessage.create({ field: `foo/${substitution.unencoded}/bar` })
  const httpRequest = transcode("TestPathVariableWithStar", requestMessage)

  expect(httpRequest.method).toEqual("GET")
  expect(httpRequest.urlEncodedPath).toEqual(`/v1/foo/${substitution.percentEncoded}/bar/baz`)
  expect(httpRequest.urlEncodedQueryString).toBeUndefined()
  expect(httpRequest.body).toBeUndefined()
})
...
test("TestPathVariableWithDoubleStar substitution", () => {
  const substitution = randomString()
  const requestMessage = TestApi.SimpleFieldMessage.create({ field: `foo/bar/baz/${substitution.unencoded}` })
  const httpRequest = transcode("TestPathVariableWithDoubleStar", requestMessage)

  expect(httpRequest.method).toEqual("GET")
  expect(httpRequest.urlEncodedPath).toEqual(`/v1/foo/bar/baz/${substitution.percentEncoded}:verb`)
  expect(httpRequest.urlEncodedQueryString).toBeUndefined()
  expect(httpRequest.body).toBeUndefined()
})
