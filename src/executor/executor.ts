...
const getFetchUrl = (baseUrl: string) => (context: ExecutionContext): ExecutionContext => {
  const queryString = context.transcodedRequest?.urlEncodedQueryString ?? ""
...
const createHttpExecutor = (
  ...
  const serviceDefinition = descriptorRoot.lookupService(service.name)

  const initialContext: Either<Error, ExecutionContext> = E.tryCatch(() => {
    const requestType = serviceDefinition.lookupType(serviceDefinition.methods[method.name].requestType)
    const requestMessage = requestType.decode(requestData)
    const responseType = serviceDefinition.lookupType(serviceDefinition.methods[method.name].responseType)
    return {
      requestMessage,
      responseType,
    }
